...
SPEED_REG_ADDR = 0x0008
SPEED_MIN = 0
SPEED_MAX = 1000
SPEED_STEP = 50
SPEED_WRITE_DELAY = 0.02
...
def set_speed(value: int, slave: int = 0x01):
    value = max(SPEED_MIN, min(SPEED_MAX, int(value)))
    pkt = build_write_single_register(slave, SPEED_REG_ADDR, value)
    plc.write(pkt)
    sleep(SPEED_WRITE_DELAY)
    print(f"[PLC] Speed set to: {value}")
    ...
    try:
        while True:
            ...
            if cmd == 'f':
                ...
                speed_cmd = min(SPEED_MAX, speed_cmd + SPEED_STEP)
                set_speed(speed_cmd)

            elif cmd == 's':
                speed_cmd = max(SPEED_MIN, speed_cmd - SPEED_STEP)
                set_speed(speed_cmd)
